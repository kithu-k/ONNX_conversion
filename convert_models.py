    ...
    for file_name in model_files:
        ...
        # ---------------------------------------------------------
            ...
            try:
                ...
                try:
                    ...
                    # Fallback to standard pickling if it's a legacy full-model save
                ...
                if is_torchscript:
                    # TorchScript stores input details in its graph structure
                    # Default to 3 if complex, or safely fallback
                    detected_channels = 3 
                else:
                ...
                try:
                    torch.onnx.export(
                        ...
                        dynamo=False  # <--- FORCE LEGACY STABLE MODE
                    )
                except TypeError:
                    # Backward-compatibility fallback if running on an older environment 
                    # where the 'dynamo' keyword argument doesn't exist yet
                    torch.onnx.export(
                        model, 
                        dummy_tensor, 
                        output_onnx_path, 
        ...
        # ---------------------------------------------------------
            ...
            try:
                ...
                # Optional feature: disable zipmap if deploying to an engine that dislikes dict outputs
